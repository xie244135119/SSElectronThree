...
class ThreeJs {
	...
	destroy(loop = true) {
		...
		if (this.threeScene !== null) {
			...
			if (this.threeRenderer.info.programs.length !== 0) {
				...
				console.log(
					"scene geometries has not released",
					this.threeRenderer.info
				);
	...
	addSky = (skys = []) => {
		...
		const hdrLoader = new THREE.CubeTextureLoader(
		...
		if (!animate) {
			...
			this.useTweenAnimate(
				...
				(e) => {
					...
					this.threeOrbitControl.target.set(
						e.orbitControl_x,
						e.orbitControl_y,
						e.orbitControl_z
					);
	...
	loadModelQueue = (
		...
		const resuseBlock = () => {
			...
			if (
				queueIndex !== 0 &&
				Object.getOwnPropertyNames(modelqueue).length === 0
			) {
				onComplete?.(objList);
				return;
			}
	...
	loadObj = (aObjPath = "", aMaterialPath = "", addToScene = true) => {
		...
		return LoadingManager.shareInstance
			.getModelDataByUrl(aMaterialPath)
			.then((materialdata) => {
				const mtlloader = new MTLLoader(
					LoadingManager.shareInstance.threeLoadingManager
				);
				const mtltext = String.fromCharCode.apply(
					null,
					new Uint8Array(materialdata)
				);
				const materials = mtlloader.parse(
					mtltext,
					getDirectoryText(aMaterialPath)
				);
				materials.shading = THREE.FlatShading;
				return LoadingManager.shareInstance
					.getModelDataByUrl(aObjPath)
					.then((objdata) => {
	...
	#addOrbitControl = (
		...
		aDomElement = document.getElementById()
	) => {
	...
	getModelsByPoint = (
		...
		const x =
			((aPoint.x - canvas.getBoundingClientRect().left) / canvas.offsetWidth) *
				2 -
			1; // 规范设施横坐标
		const y =
			-((aPoint.y - canvas.getBoundingClientRect().top) / canvas.offsetHeight) *
				2 +
			1; // 规范设施纵坐标
	...
	createOptionss = (points, material) => {
		...
		options = pro.options;
		const geometry1 = new THREE.ExtrudeBufferGeometry(
			this.createShapes(),
			options
		);
		...
		geometry.setAttribute(
			"position",
			new THREE.BufferAttribute(verticesByThree, 3)
		);
		...
		const pointsGroupBy63 = pointsGroupBy18.map((item) =>
			new Array(item.length / 3)
				.fill(0)
				.map((_, i) => item.slice(i * 3, (i + 1) * 3))
		);
